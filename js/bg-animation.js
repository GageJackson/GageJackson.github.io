...
const circles = [];
const velocity = 1;
const size = Math.min(canvas.width, canvas.height) * 6 / numCircles;
...
function initializeCircles() {
    for (let i = 0; i < numCircles; i++) {
        ...
        const gradient = ctx.createRadialGradient(
            canvas.width / 2, 0, 0, canvas.width / 2, 0, Math.max(canvas.height, canvas.width)
        );
...
function update() {
    for (const circle of circles) {
        ...
        if (circle.x <= circle.size || circle.x >= canvas.width - circle.size) {
            circle.vx *= -1;
            circle.x = circle.x <= circle.size ? circle.size : canvas.width - circle.size;
        }

        //Vertical Boundaries
        if (circle.y <= circle.size || circle.y >= canvas.height - circle.size) {
            circle.vy *= -1;
            circle.y = circle.y <= circle.size ? circle.size : canvas.height - circle.size;
        }
    }
}
